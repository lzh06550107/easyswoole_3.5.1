...
class ParserDown
{
    ...
    protected function blockNotice($Line)
    {
        //$Line =  ::: tip text contenxt :::
        //此行完整  complete = true
        if(preg_match('/^\:\:\:\s+([a-zA-Z]*)[\s]+([\s\S]*[^\s]+)[\s]+\:\:\:$/',$Line['text'], $matches)){
            $Element = array(
                'name' => 'div',
                'text' => $matches[2],
                'complete'=>true
            ...
            if (isset($matches[1]))
            {
                $noticeType = substr($matches[1], 0, strcspn($matches[1], " \t\n\f\r"));
                $Element['attributes'] = array(
                    'class' =>"custom-block {$noticeType}",
                );
            }
            $Block = array(
                'char' => $Line['text'][0],
