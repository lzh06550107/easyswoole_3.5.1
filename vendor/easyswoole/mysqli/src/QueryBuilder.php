...
class QueryBuilder
{
    ...
    /**
     * 查询构建器构造器
     * @param bool $isSubQuery 是否是子查询
     * @param string|null $subQueryAlias 子查询别名
     */
    ...
    /**
     * OrderBy条件
     * select * from table order by field(name,'seiki','iris') desc; // 按name字段值
     * https://www.cnblogs.com/chenhaoyu/p/9771012.html
     * @param $orderByField
    ...
    public function orderBy($orderByField, $orderbyDirection = "DESC", $customFieldsOrRegExp = null)
    {
        ...
        $orderByField = preg_replace("/[^ -a-z0-9\.\(\),_`\*\'\"%`]+/i", '', $orderByField); // 消毒
        // Add table prefix to orderByField if needed.
        //FIXME: We are adding prefix only if table is enclosed into `` to distinguish aliases
        // from table names
        $orderByField = preg_replace('/(\`)([`a-zA-Z0-9_%`\']*\.)/', '\1' . $this->prefix . '\2', $orderByField); // 消毒
        if (empty($orderbyDirection) || !in_array($orderbyDirection, $allowedDirection)) {
            throw new Exception('Wrong order direction: ' . $orderbyDirection);
        }
        if (is_array($customFieldsOrRegExp)) {
            foreach ($customFieldsOrRegExp as $key => $value) {
                $customFieldsOrRegExp[$key] = preg_replace("/[^\x80-\xff-a-z0-9\.\(\),_` %\']+/i", '', $value); // 消毒
            }
    ...
    {
        $groupByField = preg_replace("/[^-a-z0-9\.\(\),_\* <>=!%`\']+/i", '', $groupByField); // 消毒
